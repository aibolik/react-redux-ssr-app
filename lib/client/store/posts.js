...
const ACTION_HANDLERS = {
  [FETCH_POSTS]: (state, action) => _extends({}, state, {
    loading: true
  }),

  [UPDATE_POSTS]: (state, action) => _extends({}, state, {
    loading: false,
    items: action.payload
  }),

  [CREATE_POST]: (state, action) => _extends({}, state, {
    loading: true
  }),

  [APPEND_POST]: (state, action) => _extends({}, state, {
    loading: false,
    items: [...state.items, action.payload]
  }),

  [REMOVE_POST]: (state, action) => {
    let payload = _extends({}, state);
...
export function* saga() {
  yield all([watchFetchPosts(), watchCreatePost(), watchRemovePost()]);
}
